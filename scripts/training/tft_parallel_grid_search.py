...
from scripts.training.train_tft import compute_solar_weights, load_and_engineer_features, persist_processed
...
def train_tft_trial(config: Dict, checkpoint_dir=None):
    ...
    past_unknown = [
        c for c in df.columns if c.startswith(("pv_lag", "ghi_lag", "dni_lag", "dhi_lag")) or "_roll" in c or c == "kc"
    ]
    ...
    print(
        f"[TRIAL DEBUG] Building TFT model with config: hidden={config['hidden_size']}, heads={config['attention_heads']}",
        flush=True,
    )

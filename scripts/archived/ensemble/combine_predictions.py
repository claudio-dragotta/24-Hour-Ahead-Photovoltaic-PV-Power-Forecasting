...
def combine_predictions(
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info(f"[BEST] BEST ENSEMBLE: w_MB={best_weight:.2f}, w_TFT={1-best_weight:.2f}")
    ...
    if best_rmse < reference_rmse:
        ...
        logger.info(f"[SUCCESS] BEAT REFERENCE! Improvement: {improvement_vs_ref:+.2f}%")
    else:
        gap_vs_ref = (best_rmse - reference_rmse) / reference_rmse * 100
        logger.info(f"[INFO] Gap vs reference (3.17 kW): {gap_vs_ref:+.2f}%")
    logger.info("=" * 60)

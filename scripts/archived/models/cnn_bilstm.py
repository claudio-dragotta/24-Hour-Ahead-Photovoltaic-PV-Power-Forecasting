...
def build_cnn_bilstm_fusion_attention(
    ...
    weather_hist_features: int,
    weather_forecast_features: int = None,
    horizon: int = 24,
    ...
    Args:
        ...
        weather_hist_features: Number of historical weather features
        weather_forecast_features: Number of forecast weather features (defaults to weather_hist_features)
        horizon: Output forecast horizon
        embedding_dim: Embedding dimension for attention
        num_attention_heads: Number of attention heads
        dropout_rate: Dropout rate for regularization
        pv_features: Number of PV features (default 1)

    Returns:
        Compiled multi-input Keras model
    """
    if weather_forecast_features is None:
        weather_forecast_features = weather_hist_features

    # ===== BRANCH 1: PV History =====
    pv_input = keras.Input(shape=(pv_seq_len, pv_features), name="pv_history")
    pv = layers.Conv1D(64, kernel_size=5, padding="same", activation="relu")(pv_input)
    ...
    weather_hist_input = keras.Input(shape=(weather_seq_len, weather_hist_features), name="weather_history")
    ...
    weather_forecast_input = keras.Input(
        shape=(weather_forecast_len, weather_forecast_features), name="weather_forecast"
    )
    weather_forecast = layers.Dense(
        embedding_dim,
        activation="relu",
